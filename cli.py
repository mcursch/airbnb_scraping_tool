...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Acquire only; make no DB writes and exit 0",
)
@click.option(
    "--no-extract",
    is_flag=True,
    default=False,
    help="Persist RawScrape rows (status='pending') but skip LLM extraction",
)
def scan(
    area: str,
    checkin: str | None,
    ...
    Pass --dry-run to run the scrapers without writing anything to the database.
    Pass --no-extract to save raw payloads but skip the LLM extraction step.
    """
    ...
    # Resolve extractor — skip LLM client if extraction is not needed
    if dry_run or no_extract:
        ...
        try:
            ...
            click.echo(
                f"Warning: could not initialise Anthropic client ({exc}). "
                "Pass --no-extract or set ANTHROPIC_API_KEY.",
                err=True,
            )
            sys.exit(1)
    ...
    run_id = pipeline.run(query, dry_run=dry_run, no_extract=no_extract)

    if dry_run:
        click.echo("Dry run complete. No database rows written.")
    else:
